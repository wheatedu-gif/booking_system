-- =========================================================
-- 智慧預約系統 - 終極全功能整合初始化腳本 (V15 營運與維運強化版)
-- =========================================================
...
DROP TABLE IF EXISTS email_logs CASCADE; -- [新增] 紀錄表
...
CREATE TABLE email_logs (
  ...
  subject TEXT,
  type TEXT, -- test, new, confirmed, etc.
...
ALTER TABLE email_logs ENABLE ROW LEVEL SECURITY; -- 啟用 RLS
...
CREATE POLICY "Admin Full Access Cust" ON customers FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin Full Access Apt" ON appointments FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin Full Access Logs" ON email_logs FOR ALL USING (auth.role() = 'authenticated'); -- 管理員看紀錄
CREATE POLICY "Admin Full Access Content" ON page_content FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin Full Access Forms" ON form_definitions FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin Full Access Business" ON business_hours FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin Full Access Special" ON special_dates FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Admin Full Access Settings" ON system_settings FOR ALL USING (auth.role() = 'authenticated');
...
('booking_form', '[{"id": "sys_date", "name": "date", "label": "預約日期", "type": "date", "required": true, "isSystem": true}]'::jsonb);
...
('email_templates', '{"new_booking": {"subject": "收到預約申請", "body": "您好 {name}，預約待處理中。"}, "confirmed": {"subject": "預約確認", "body": "您好 {name}，預約已確認！"}, "completed": {"subject": "感謝您的光臨", "body": "您好 {name}，感謝光臨！"}}'::jsonb);
