...
export const BookingPage: React.FC = () => {
  ...
  const [rules, setRules] = useState({ 
    ...
    min_lead_time_hours: 2, // 預設需提前 2 小時
  ...
  if (successId) return (
    <div className="max-w-md mx-auto mt-20 text-center p-10 bg-white rounded-[3rem] shadow-xl animate-in zoom-in-95">
        ...
        <p className="text-slate-500 mb-8">我們已收到您的申請，請靜候確認通知。</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-8">
            <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100">
                ...
                <div className="space-y-8">
                    ...
                    <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 block ml-1">2. 選擇可用時段 (需提前 {rules.min_lead_time_hours} 小時)</label>
                        ...
                        {availableSlots.length === 0 && <p className="text-center text-slate-300 py-10 font-bold italic">此日期目前無符合條件的時段</p>}
                    </div>
                </div>
            </div>
        </div>
        <div className="space-y-8">
            <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100 sticky top-24">
                <h2 className="text-2xl font-black mb-8 flex items-center gap-3"><FileText className="text-blue-600"/> 填寫預約資料</h2>
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <button type="submit" disabled={submitting || !bookingTime} className="w-full btn-primary py-5 rounded-2xl font-black shadow-xl shadow-blue-200 disabled:opacity-30 transition-all">{submitting ? '提交中...' : '確認送出'}</button>
                </form>
            </div>
        </div>
      </div>
