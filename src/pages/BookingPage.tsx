  ...
  return (
    <div className="max-w-3xl mx-auto py-12 px-4">
      ...
      <div className="bg-white rounded-[3rem] shadow-2xl shadow-slate-200/50 overflow-hidden border border-slate-100">
        <div className="bg-blue-600 p-10 text-white"><h1 className="text-3xl font-black flex items-center gap-3 tracking-tight"><Calendar size={32} /> 預約服務</h1><p className="text-blue-100 mt-2 text-lg font-medium opacity-80">請選擇日期與時段</p></div>
        <form onSubmit={handleSubmit} className="p-10 space-y-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
            <div><label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 ml-1">{getLabel('date', '預約日期')} <span className="text-red-500">*</span></label><input type="date" required className="input-field bg-slate-50 border-none rounded-2xl py-4 focus:bg-white shadow-inner" value={bookingDate} min={format(new Date(), 'yyyy-MM-dd')} max={maxDateStr} onChange={(e) => setBookingDate(e.target.value)} /></div>
            <div><label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 ml-1">{getLabel('time', '預約時間')} <span className="text-red-500">*</span></label>{availableSlots.length > 0 ? (<select className="input-field bg-slate-50 border-none rounded-2xl py-4 focus:bg-white shadow-inner appearance-none" value={bookingTime} onChange={(e) => setBookingTime(e.target.value)}>{availableSlots.map(t => <option key={t} value={t}>{t}</option>)}</select>) : <div className="p-4 bg-red-50 text-red-500 rounded-2xl text-sm font-bold border border-red-100">本日已無可用時段</div>}</div>
          </div>
          {bookingDef?.fields?.filter((f:any) => !f.isSystem).map((field: any) => (
            <div key={field.id} className="mt-4"><label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 ml-1">{field.label} {field.required && <span className="text-red-500">*</span>}</label>{field.type === 'select' ? (<select required={field.required} className="input-field bg-slate-50 border-none rounded-2xl py-4 shadow-inner appearance-none" onChange={(e) => setFormData({ ...formData, [field.label]: e.target.value })}><option value="">請選擇...</option>{field.options?.map((opt: string) => <option key={opt} value={opt}>{opt}</option>)}</select>) : <input type={field.type} required={field.required} className="input-field bg-slate-50 border-none rounded-2xl py-4 shadow-inner" onChange={(e) => setFormData({ ...formData, [field.label]: e.target.value })} />}</div>
          ))}
          <button type="submit" disabled={submitting} className="w-full btn-primary py-5 rounded-2xl text-xl font-black shadow-xl shadow-blue-200 transform transition-all active:scale-[0.98] disabled:opacity-50">{submitting ? '提交預約中...' : '確認預約'}</button>
        </form>
      </div>
    </div>
  );
