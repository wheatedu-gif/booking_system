...
const SettingsManager: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center"><h2 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3"><Settings className="text-blue-600" /> 通知系統設定</h2><div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
        <button onClick={() => setActiveSubTab('smtp')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'smtp' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>SMTP</button>
        <button onClick={() => setActiveSubTab('templates')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'templates' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>範本</button>
        <button onClick={() => setActiveSubTab('logs')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'logs' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>日誌</button>
      </div></div>
