...
import { Plus, Trash2, Save, Settings, Users, Calendar as CalendarIcon, FormInput, Clock, LayoutTemplate, List, ChevronLeft, ChevronRight, Lock, AlertCircle, Download, Send, Edit3, X, TrendingUp, Search, ExternalLink, LayoutDashboard, FileText, StickyNote, History, CheckCircle2, CalendarPlus, BarChart3, KeyRound, Mail } from 'lucide-react';
...
const ManualBookingModal: React.FC<{ onClose: () => void, onRefresh: () => void, initialCustomerId?: string, formDefs: FormDefinition[] }> = ({ onClose, onRefresh, initialCustomerId, formDefs }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); if (!selectedCustomer || !time) return; setLoading(true); const { error } = await supabase.from('appointments').insert([{ customer_id: selectedCustomer, booking_date: date, booking_time: time, status: 'confirmed', source: 'manual', booking_data: dynamicData }]); if (error) showToast('建立失敗', 'error'); else { showToast('預約已建立'); onRefresh(); onClose(); } setLoading(false); };
...
const SettingsManager: React.FC = () => {
  const [activeSubTab, setActiveSubSubTab] = useState<'smtp' | 'templates' | 'logs'>('smtp');
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center"><h2 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3"><Settings className="text-blue-600" /> 通知系統設定</h2><div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200"><button onClick={() => setActiveSubSubTab('smtp')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'smtp' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>SMTP</button><button onClick={() => setActiveSubSubTab('templates')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'templates' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>範本</button><button onClick={() => setActiveSubSubTab('logs')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'logs' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>日誌</button></div></div>
