...
import { Plus, Trash2, Save, Settings, Users, Calendar as CalendarIcon, FormInput, Clock, LayoutTemplate, List, ChevronLeft, ChevronRight, Lock, AlertCircle, Download, Send, Edit3, X, TrendingUp, Search, ExternalLink, LayoutDashboard, FileText, StickyNote, History, CheckCircle2 } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      if (activeTab === 'appointments' || activeTab === 'home' || activeTab === 'customers') {
  ...
  const updateAppointmentStatus = async (id: string, status: string, reason?: string) => {
    ...
    if (error) showToast('更新失敗', 'error');
      ...
      showToast(status === 'confirmed' ? '已確認' : status === 'completed' ? '已標記為完成' : '已取消');
        ...
        <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                ...
                <div className="space-y-3">
                    ...
                    {appointments.filter(a => a.status === 'pending').length === 0 && <p className="text-center text-slate-300 py-4 font-bold italic">目前沒有待處理的預約</p>}
                </div>
            </div>
        </div>
    );
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {viewMode === 'calendar' ? <AppointmentCalendar appointments={filteredApts} onStatusChange={onStatusChange} onSelect={setSelectedApt} /> : (
        <div className="overflow-hidden border border-slate-100 rounded-3xl"><table className="w-full text-left border-collapse"><thead className="bg-slate-50/50"><tr><th className="py-5 px-6 text-xs font-bold text-slate-400 uppercase tracking-widest">時間</th><th className="py-5 px-6 text-xs font-bold text-slate-400 uppercase tracking-widest">客戶</th><th className="py-5 px-6 text-xs font-bold text-slate-400 uppercase tracking-widest">狀態</th><th className="py-5 px-6 text-xs font-bold text-slate-400 uppercase tracking-widest text-right">動作</th></tr></thead><tbody className="divide-y divide-slate-50">{filteredApts.map(apt => (
                <tr key={apt.id} className="hover:bg-slate-50/50 transition-colors group cursor-pointer" onClick={() => setSelectedApt(apt)}><td className="py-5 px-6"><div className="font-bold text-slate-700">{apt.booking_date}</div><div className="text-blue-500 text-xs font-medium">{apt.booking_time.slice(0,5)}</div></td><td className="py-5 px-6"><div className="font-bold text-slate-700 group-hover:text-blue-600 transition-all flex items-center gap-2">{(apt as any).customers?.full_name} <ExternalLink size={12} className="opacity-0 group-hover:opacity-100" /></div><div className="text-slate-400 text-xs font-medium">{(apt as any).customers?.email}</div></td><td className="py-5 px-6">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${apt.status === 'confirmed' ? 'bg-green-100 text-green-800' : apt.status === 'completed' ? 'bg-slate-100 text-slate-600' : apt.status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{apt.status === 'confirmed' ? '已確認' : apt.status === 'completed' ? '已完成' : apt.status === 'cancelled' ? '已取消' : '待處理'}</span>
                </td><td className="py-5 px-6 text-right" onClick={e => e.stopPropagation()}><div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all">
                    ...
                    {apt.status === 'confirmed' && isPast(parseISO(apt.booking_date)) && <button onClick={() => onStatusChange(apt.id, 'completed')} className="bg-blue-600 text-white px-4 py-1.5 rounded-xl text-xs font-bold hover:bg-blue-700">設為完成</button>}
...
const CustomerManager: React.FC<{ customers: any[], onRefresh: () => void, allAppointments: Appointment[] }> = ({ customers, onRefresh, allAppointments }) => {
  ...
  return (
    <div className="space-y-8 relative animate-in fade-in duration-500"><div className="flex justify-between items-center"><h2 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3"><Users className="text-blue-600" /> 會員資料庫</h2><button onClick={() => {}} className="flex items-center gap-2 px-6 py-2 bg-slate-50 text-slate-600 rounded-2xl font-bold text-sm border border-slate-100 hover:bg-white transition-all shadow-sm"><Download size={18}/> 匯出 CSV</button></div><div className="grid gap-4">{customers.map(c => (
      ...
      {viewHistory && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4" onClick={() => setViewHistory(null)}><div className="bg-white rounded-[3rem] w-full max-w-2xl max-h-[80vh] overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}><div className="bg-blue-600 p-8 text-white flex justify-between items-center"><div className="flex items-center gap-4"><div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center font-bold text-xl">{viewHistory.full_name[0]}</div><div><h3 className="text-xl font-black">{viewHistory.full_name} 的預約歷程</h3><p className="text-blue-100 text-xs">帳號：{viewHistory.email}</p></div></div><button onClick={() => setViewHistory(null)} className="p-2 hover:bg-white/10 rounded-full"><X/></button></div><div className="p-8 overflow-y-auto flex-1 space-y-4">{allAppointments.filter(a => a.customer_id === viewHistory.id).length > 0 ? allAppointments.filter(a => a.customer_id === viewHistory.id).map(a => (<div key={a.id} className="p-5 bg-slate-50 rounded-2xl flex justify-between items-center border border-slate-100"><div><div className="font-bold text-slate-800">{a.booking_date} {a.booking_time.slice(0,5)}</div><div className="text-xs text-slate-400">狀態：{a.status} | 來源：{a.source || 'online'}</div></div><span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${a.status === 'confirmed' ? 'bg-green-100 text-green-600' : a.status === 'completed' ? 'bg-slate-200 text-slate-600' : 'bg-red-100 text-red-600'}`}>{a.status}</span></div>)) : <div className="p-20 text-center text-slate-300 font-bold italic">尚無預約紀錄</div>}</div></div></div>)}
    </div>
  );
};
