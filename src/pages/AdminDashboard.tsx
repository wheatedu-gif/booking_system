        ...
        <div className="flex-1 bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden min-h-[700px]">
          {loading && activeTab !== 'cms' && activeTab !== 'availability' ? (
            ...
            <div className="p-8">
                ...
                {activeTab === 'customers' && <CustomerManager customers={customers} onRefresh={fetchData} />}
            </div>
          )}
        </div>
      </div>
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-3">
            <button onClick={() => setShowAddModal(true)} className="btn-primary flex items-center gap-2 px-6 py-2 rounded-xl text-sm font-bold shadow-lg shadow-blue-100"><Plus size={18}/> 手動新增預約</button>
            <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 shrink-0">
                <button onClick={() => setViewMode('list')} className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${viewMode === 'list' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>列表</button>
                <button onClick={() => setViewMode('calendar')} className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${viewMode === 'calendar' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>日曆</button>
            </div>
        </div>
      </div>
...
const ManualBookingModal: React.FC<{ onClose: () => void, onRefresh: () => void }> = ({ onClose, onRefresh }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault(); if (!selectedCustomer) return; setLoading(true);
        const { error } = await supabase.from('appointments').insert([{ customer_id: selectedCustomer, booking_date: date, booking_time: time, status: 'confirmed' }]);
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
            <div className="bg-white rounded-[3rem] w-full max-w-md p-10 shadow-2xl animate-in zoom-in-95 duration-200">
                <div className="flex justify-between items-center mb-8"><h3 className="text-2xl font-black text-slate-800">手動建立預約</h3><button onClick={onClose} className="p-2 text-slate-400 hover:bg-slate-100 rounded-full"><X /></button></div>
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div><label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">選擇會員</label><select className="input-field rounded-2xl py-4 bg-slate-50 border-none" value={selectedCustomer} onChange={e => setSelectedCustomer(e.target.value)} required><option value="">請選擇客戶...</option>{customers.map(c => <option key={c.id} value={c.id}>{c.full_name} ({c.email})</option>)}</select></div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">日期</label><input type="date" className="input-field rounded-2xl py-4 bg-slate-50 border-none" value={date} onChange={e => setDate(e.target.value)} required /></div>
                        <div><label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">時間</label><input type="time" className="input-field rounded-2xl py-4 bg-slate-50 border-none" value={time} onChange={e => setTime(e.target.value)} required /></div>
                    </div>
                    <button type="submit" disabled={loading} className="w-full btn-primary py-5 rounded-2xl font-black shadow-xl shadow-blue-200">建立並確認預約</button>
                </form>
            </div>
        </div>
    );
...
const FormManager: React.FC<{ formDefs: FormDefinition[], onRefresh: () => void }> = ({ formDefs, onRefresh }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500"><h2 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3"><FormInput className="text-blue-600" /> 表單自定義管理</h2>{!editingDef ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">{formDefs.map(def => (
          ...
          ))}</div>) : (<div className="space-y-6"><div className="flex justify-between items-center bg-blue-600 p-8 rounded-[2.5rem] text-white shadow-xl shadow-blue-100"><div><h3 className="font-bold text-xl">正在編輯：{editingDef.type}</h3><p className="text-xs text-blue-100 mt-2 font-medium opacity-80 flex items-center gap-1"><Lock size={12}/> 系統核心欄位已鎖定類型。</p></div><div className="flex gap-3"><button onClick={() => setEditingDef(null)} className="px-6 py-2 text-white font-bold hover:bg-white/10 rounded-xl transition-all">取消</button><button onClick={handleSave} className="bg-white text-blue-600 px-10 py-3 rounded-2xl font-black shadow-lg hover:bg-blue-50 transition-all">儲存變更</button></div></div><div className="grid gap-4">{editingDef.fields.map((field: any, index) => (
            ...
            ))}</div>{editingDef.fields.some((f: any) => f.type === 'select') && <div className="p-4 bg-amber-50 text-amber-700 text-xs rounded-xl italic">提示：選擇「選單」類型後，請在下方設定選項 (以逗號分隔)。</div>}
...
const CustomerManager: React.FC<{ customers: any[], onRefresh: () => void }> = ({ customers, onRefresh }) => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    if (error) showToast('更新失敗', 'error'); else { showToast('會員資料已更新'); setEditingCustomer(null); onRefresh(); }
  };
  const handleDelete = async (id: string) => {
      if (!window.confirm('確定要刪除此會員嗎？相關預約也會一併刪除且無法復原。')) return;
      const { error } = await supabase.from('customers').delete().eq('id', id);
      if (error) showToast('刪除失敗', 'error');
      else { showToast('會員已成功刪除'); onRefresh(); }
  };
  const downloadCSV = () => {
