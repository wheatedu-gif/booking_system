...
import { Plus, Trash2, Save, Settings, Users, Calendar as CalendarIcon, FormInput, Clock, LayoutTemplate, List, ChevronLeft, ChevronRight, Lock, AlertCircle, Download, Send, Edit3, X, TrendingUp, Search, ExternalLink, LayoutDashboard, FileText, StickyNote, History } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const updateAppointmentStatus = async (id: string, status: string, reason?: string) => {
    ...
    if (error) showToast('更新失敗', 'error');
      ...
      showToast(status === 'confirmed' ? '已確認' : '已取消');
            ...
            <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                ...
                <div className="space-y-3">
                    {appointments.slice(0, 5).map(apt => (
                        <div key={apt.id} className="bg-white p-4 rounded-2xl flex justify-between items-center shadow-sm border border-white hover:shadow-md transition-all">
...
const AppointmentManager: React.FC<{ appointments: Appointment[], onStatusChange: (id: string, s: string, reason?: string) => void, onRefresh: () => void }> = ({ appointments, onStatusChange, onRefresh }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4"><div className="relative w-full md:w-96"><input type="text" placeholder="搜尋姓名、Email 或日期..." className="input-field pl-12 py-3 bg-slate-50 border-none rounded-2xl w-full focus:bg-white transition-all shadow-inner" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /><Search size={20} className="absolute left-4 top-3.5 text-slate-300" /></div><div className="flex gap-3"><button onClick={() => setShowAddModal(true)} className="btn-primary flex items-center gap-2 px-6 py-2 rounded-xl text-sm font-bold shadow-lg shadow-blue-100"><Plus size={18}/> 手動新增</button><div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 shrink-0"><button onClick={() => setViewMode('list')} className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${viewMode === 'list' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>列表</button><button onClick={() => setViewMode('calendar')} className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${viewMode === 'calendar' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>日曆</button></div></div></div>
...
const AppointmentDetailModal: React.FC<{ apt: Appointment, onClose: () => void, onStatusChange: (id: string, s: string, r?: string) => void, onRefresh: () => void }> = ({ apt, onClose, onStatusChange, onRefresh }) => {
    ...
    const saveNotes = async () => {
        setSavingNotes(true);
        const { error } = await supabase.from('appointments').update({ admin_notes: notes }).eq('id', apt.id);
        if (error) showToast('儲存失敗', 'error'); else { showToast('筆記已更新'); onRefresh(); }
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4" onClick={onClose}><div className="bg-white rounded-[3rem] w-full max-w-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}><div className="bg-slate-900 p-10 text-white flex justify-between items-start"><div><div className="text-blue-400 text-[10px] font-black uppercase tracking-[0.2em] mb-2">預約詳情</div><h3 className="text-3xl font-black">{customer?.full_name}</h3><p className="text-slate-400 mt-1">{customer?.email}</p></div><button onClick={onClose} className="p-3 bg-white/10 hover:bg-white/20 rounded-2xl transition-all"><X /></button></div><div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
            <section className="space-y-6">
                ...
                <div className="p-6 bg-slate-50 rounded-3xl"><h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">客戶填寫內容</h4><div className="space-y-3">{Object.entries(apt.booking_data || {}).map(([k, v]) => (<div key={k} className="flex justify-between text-sm border-b border-slate-200/50 pb-2 font-medium"><span className="text-slate-500">{k}</span><span className="text-slate-800">{String(v)}</span></div>))}</div></div>
            </section>
            <section className="space-y-6">
                <div className="flex flex-col h-full"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2"><StickyNote size={14} className="text-amber-500"/> 管理員私收筆記 (僅後台可見)</label><textarea className="input-field bg-amber-50/50 border-amber-100 rounded-2xl p-4 flex-1 min-h-[200px] text-sm" value={notes} onChange={e => setNotes(e.target.value)} placeholder="記錄此客戶的特徵、偏好或注意事項..."></textarea><button onClick={saveNotes} disabled={savingNotes} className="mt-3 bg-slate-800 text-white py-2 rounded-xl text-xs font-bold hover:bg-slate-900 transition-all">{savingNotes ? '儲存中...' : '儲存筆記'}</button></div>
            </section>
        </div><div className="bg-slate-50 p-8 flex justify-end gap-4">{apt.status === 'pending' && <button onClick={() => { onStatusChange(apt.id, 'confirmed'); onClose(); }} className="bg-green-600 text-white px-8 py-3 rounded-2xl font-black shadow-lg shadow-green-200 hover:bg-green-700 transition-all">確認預約</button>}{apt.status !== 'cancelled' && <button onClick={() => { const r = window.prompt('原因'); if(r!==null) { onStatusChange(apt.id, 'cancelled', r); onClose(); } }} className="bg-white text-red-500 border border-red-100 px-8 py-3 rounded-2xl font-black transition-all">取消預約</button>}<button onClick={onClose} className="text-slate-400 font-bold px-4 hover:text-slate-600">關閉</button></div></div></div>
    );
};

const ManualBookingModal: React.FC<{ onClose: () => void, onRefresh: () => void }> = ({ onClose, onRefresh }) => {
    const [customers, setCustomers] = useState<any[]>([]);
    ...
    const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); if (!selectedCustomer) return; setLoading(true); const { error } = await supabase.from('appointments').insert([{ customer_id: selectedCustomer, booking_date: date, booking_time: time, status: 'confirmed', source: 'manual' }]); if (error) showToast('建立失敗', 'error'); else { showToast('預約已建立'); onRefresh(); onClose(); } setLoading(false); };
    return (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4"><div className="bg-white rounded-[3rem] w-full max-w-md p-10 shadow-2xl animate-in zoom-in-95 duration-200"><div className="flex justify-between items-center mb-8"><h3 className="text-2xl font-black text-slate-800">手動建立預約</h3><button onClick={onClose} className="p-2 text-slate-400 hover:bg-slate-100 rounded-full"><X /></button></div><form onSubmit={handleSubmit} className="space-y-6"><div><label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">選擇會員</label><select className="input-field rounded-2xl py-4 bg-slate-50 border-none" value={selectedCustomer} onChange={e => setSelectedCustomer(e.target.value)} required><option value="">請選擇客戶...</option>{customers.map(c => <option key={c.id} value={c.id}>{c.full_name} ({c.email})</option>)}</select></div><div className="grid grid-cols-2 gap-4"><div><label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">日期</label><input type="date" className="input-field rounded-2xl py-4 bg-slate-50 border-none" value={date} onChange={e => setDate(e.target.value)} required /></div><div><label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">時間</label><input type="time" className="input-field rounded-2xl py-4 bg-slate-50 border-none" value={time} onChange={e => setTime(e.target.value)} required /></div></div><button type="submit" disabled={loading} className="w-full btn-primary py-5 rounded-2xl font-black shadow-xl shadow-blue-200">建立並確認預約</button></form></div></div>);
};
...
const CustomerManager: React.FC<{ customers: any[], onRefresh: () => void, allAppointments: Appointment[] }> = ({ customers, onRefresh, allAppointments }) => {
  ...
  return (
    <div className="space-y-8 relative animate-in fade-in duration-500"><div className="flex justify-between items-center"><h2 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3"><Users className="text-blue-600" /> 會員資料庫</h2><button onClick={() => {}} className="flex items-center gap-2 px-6 py-2 bg-slate-50 text-slate-600 rounded-2xl font-bold text-sm border border-slate-100 hover:bg-white transition-all shadow-sm"><Download size={18}/> 匯出 CSV</button></div><div className="grid gap-4">{customers.map(c => (
      ...
      {viewHistory && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4" onClick={() => setViewHistory(null)}><div className="bg-white rounded-[3rem] w-full max-w-2xl max-h-[80vh] overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}><div className="bg-blue-600 p-8 text-white flex justify-between items-center"><div className="flex items-center gap-4"><div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center font-bold text-xl">{viewHistory.full_name[0]}</div><div><h3 className="text-xl font-black">{viewHistory.full_name} 的預約歷程</h3><p className="text-blue-100 text-xs">帳號：{viewHistory.email}</p></div></div><button onClick={() => setViewHistory(null)} className="p-2 hover:bg-white/10 rounded-full"><X/></button></div><div className="p-8 overflow-y-auto flex-1 space-y-4">{allAppointments.filter(a => a.customer_id === viewHistory.id).length > 0 ? allAppointments.filter(a => a.customer_id === viewHistory.id).map(a => (<div key={a.id} className="p-5 bg-slate-50 rounded-2xl flex justify-between items-center border border-slate-100"><div><div className="font-bold text-slate-800">{a.booking_date} {a.booking_time.slice(0,5)}</div><div className="text-xs text-slate-400">狀態：{a.status} | 來源：{a.source || 'online'}</div></div><span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${a.status === 'confirmed' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>{a.status}</span></div>)) : <div className="p-20 text-center text-slate-300 font-bold italic">尚無預約紀錄</div>}</div></div></div>)}
    </div>
  );
};

// ... (其餘 FormManager, SettingsManager, AppointmentCalendar 保持一致) ...
