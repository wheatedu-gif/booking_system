...
import { Plus, Trash2, Save, Settings, Users, Calendar as CalendarIcon, FormInput, Clock, LayoutTemplate, List, ChevronLeft, ChevronRight, Lock, AlertCircle, Download, Send, Edit3, X, TrendingUp, Search, ExternalLink, LayoutDashboard, FileText, StickyNote, History, CheckCircle2, CalendarPlus } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (activeTab === 'forms') { const { data } = await supabase.from('form_definitions').select('*').order('type'); setFormDefs(data || []); }
      if (activeTab === 'customers' || activeTab === 'home') { const { data } = await supabase.from('customers').select('*').order('created_at', { ascending: false }); setCustomers(data || []); }
...
const DashboardHome: React.FC<{ appointments: Appointment[], customers: any[] }> = ({ appointments, customers }) => {
    ...
    const getW = (s: string) => (appointments.filter(a => a.status === s).length / total) * 100 + '%';
    return (
        <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                ...
                <div className="h-4 w-full bg-slate-200 rounded-full overflow-hidden flex shadow-inner">
                    <div style={{ width: getW('completed') }} className="bg-green-500 h-full"></div>
                    <div style={{ width: getW('confirmed') }} className="bg-blue-500 h-full"></div>
                    <div style={{ width: getW('pending') }} className="bg-amber-500 h-full"></div>
                    <div style={{ width: getW('cancelled') }} className="bg-red-400 h-full"></div>
                </div>
                <div className="flex gap-6 mt-4">
                    {['completed', 'confirmed', 'pending', 'cancelled'].map(s => (
                        <div key={s} className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400">
                            <div className={`w-2 h-2 rounded-full ${s === 'completed' ? 'bg-green-500' : s === 'confirmed' ? 'bg-blue-500' : s === 'pending' ? 'bg-amber-500' : 'bg-red-400'}`}></div>{s}
                        </div>
                    ))}
                </div>
            </div>
...
const SettingsManager: React.FC = () => {
  ...
  const saveSettings = async () => { await supabase.from('system_settings').upsert([{ key: 'email_config', value: config }, { key: 'email_templates', value: templates }]); showToast('設定已儲存'); };
