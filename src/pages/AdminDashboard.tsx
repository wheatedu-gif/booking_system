...
import { Plus, Trash2, Save, Settings, Users, Calendar as CalendarIcon, FormInput, Clock, LayoutTemplate, List, ChevronLeft, ChevronRight, Lock, AlertCircle, Download, Send, Edit3, X, TrendingUp, Search, ExternalLink, LayoutDashboard, FileText, StickyNote, History, CheckCircle2, CalendarPlus, BarChart3, KeyRound, Filter } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden min-h-[700px]">
          {loading && activeTab !== 'cms' && activeTab !== 'availability' && activeTab !== 'settings' ? (
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="relative w-full md:w-96"><input type="text" placeholder="搜尋姓名、日期..." className="input-field pl-12 py-3 bg-slate-50 border-none rounded-2xl w-full focus:bg-white transition-all shadow-inner" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /><Search size={20} className="absolute left-4 top-3.5 text-slate-300" /></div>
        <div className="flex gap-3">
            <button onClick={() => setShowAddModal(true)} className="btn-primary flex items-center gap-2 px-6 py-2 rounded-xl text-sm font-bold shadow-lg shadow-blue-100"><Plus size={18}/> 新增預約</button>
      ...
      <div className="flex flex-wrap gap-2">
          {['all', 'pending', 'confirmed', 'completed', 'cancelled'].map(s => (
              <button 
                ...
                className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all ${statusFilter === s ? 'bg-slate-800 text-white border-slate-800 shadow-md' : 'bg-white text-slate-400 border-slate-100 hover:border-blue-200'}`}
              >
                  {s === 'all' ? '全部' : s}
              </button>
          ))}
...
const ManualBookingModal: React.FC<{ onClose: () => void, onRefresh: () => void, initialCustomerId?: string, formDefs: FormDefinition[] }> = ({ onClose, onRefresh, initialCustomerId, formDefs }) => {
    ...
    return (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4"><div className="bg-white rounded-[3rem] w-full max-w-xl max-h-[90vh] overflow-y-auto p-10 shadow-2xl animate-in zoom-in-95 duration-200"><div className="flex justify-between items-center mb-8"><h3 className="text-2xl font-black text-slate-800">手動建立預約</h3><button onClick={onClose} className="p-2 text-slate-400 hover:bg-slate-100 rounded-full"><X /></button></div><form onSubmit={handleSubmit} className="space-y-6"><div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1">1. 選擇會員</label><select className="input-field rounded-2xl py-4 bg-slate-50 border-none shadow-inner" value={selectedCustomer} onChange={e => setSelectedCustomer(e.target.value)} required><option value="">請選擇客戶...</option>{customers.map(c => <option key={c.id} value={c.id}>{c.full_name} ({c.email})</option>)}</select></div><div className="grid grid-cols-2 gap-4"><div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1">2. 日期</label><input type="date" className="input-field rounded-2xl py-4 bg-slate-50 border-none shadow-inner" value={date} onChange={e => setDate(e.target.value)} required /></div><div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1">3. 時段</label><select className="input-field rounded-2xl py-4 bg-slate-50 border-none shadow-inner" value={time} onChange={e => setTime(e.target.value)} required disabled={availableSlots.length === 0}><option value="">請選擇時段...</option>{availableSlots.map(s => <option key={s} value={s}>{s}</option>)}</select></div></div><div className="pt-4 border-t border-slate-50"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 block ml-1">4. 填寫詳細資料</label><div className="space-y-4">{bookingFormDef?.fields.filter(f => !f.isSystem).map(field => (
                    ...
                    ))}</div></div><button type="submit" disabled={loading || !time} className="w-full btn-primary py-5 rounded-2xl font-black shadow-xl shadow-blue-200">建立預約</button></form></div></div>);
};
...
const AppointmentDetailModal: React.FC<{ apt: Appointment, onClose: () => void, onStatusChange: (id: string, s: string, r?: string) => void, onRefresh: () => void }> = ({ apt, onClose, onStatusChange, onRefresh }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4" onClick={onClose}><div className="bg-white rounded-[3rem] w-full max-w-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}><div className="bg-slate-900 p-10 text-white flex justify-between items-start"><div><div className="text-blue-400 text-[10px] font-black uppercase tracking-[0.2em] mb-2">預約詳情</div><h3 className="text-3xl font-black">{customer?.full_name}</h3><p className="text-slate-400 mt-1">{customer?.email}</p></div><button onClick={onClose} className="p-3 bg-white/10 hover:bg-white/20 rounded-2xl transition-all"><X /></button></div><div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10"><section className="space-y-6"><div className="flex gap-4 items-start"><Clock className="text-blue-600 mt-1"/><div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">預約時間</div><div className="text-lg font-bold">{apt.booking_date} {apt.booking_time.slice(0,5)}</div></div><div className="flex gap-4 items-start"><AlertCircle className="text-blue-600 mt-1"/><div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">當前狀態</div><div className="text-lg font-bold uppercase">{apt.status}</div></div><div className="p-6 bg-slate-50 rounded-3xl"><h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">填寫內容</h4><div className="space-y-3">{Object.entries(apt.booking_data || {}).map(([k, v]) => (<div key={k} className="flex justify-between text-sm border-b border-slate-200/50 pb-2 font-medium"><span className="text-slate-500 font-bold">{k}</span><span className="text-slate-800">{String(v)}</span></div>))}</div></div></section><section className="space-y-6"><div className="flex flex-col h-full"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2"><StickyNote size={14} className="text-amber-500"/> 管理員筆記</label><textarea className="input-field bg-amber-50/50 border-amber-100 rounded-2xl p-4 flex-1 min-h-[200px] text-sm shadow-inner" value={notes} onChange={e => setNotes(e.target.value)} placeholder="記錄客戶偏好..."></textarea><button onClick={saveNotes} disabled={savingNotes} className="mt-3 bg-slate-800 text-white py-2 rounded-xl text-xs font-bold hover:bg-slate-900 transition-all">{savingNotes ? '儲存中...' : '儲存筆記'}</button></div></section></div><div className="bg-slate-50 p-8 flex justify-end gap-4">
            ...
            {apt.status === 'confirmed' && <button onClick={() => { onStatusChange(apt.id, 'completed'); onClose(); }} className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-black shadow-lg hover:bg-blue-700 transition-all">標記完成</button>}
