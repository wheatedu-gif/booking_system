...
const AppointmentManager: React.FC<{ appointments: Appointment[], onStatusChange: (id: string, s: string, reason?: string) => void, onRefresh: () => void }> = ({ appointments, onStatusChange, onRefresh }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4"><div className="relative w-full md:w-96"><input type="text" placeholder="搜尋姓名、Email 或日期..." className="input-field pl-12 py-3 bg-slate-50 border-none rounded-2xl w-full focus:bg-white transition-all shadow-inner" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /><Search size={20} className="absolute left-4 top-3.5 text-slate-300" /></div><div className="flex gap-3"><button onClick={() => setShowAddModal(true)} className="btn-primary flex items-center gap-2 px-6 py-2 rounded-xl text-sm font-bold shadow-lg shadow-blue-100 transition-all active:scale-95"><Plus size={18}/> 手動新增</button><div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 shrink-0"><button onClick={() => setViewMode('list')} className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${viewMode === 'list' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>列表</button><button onClick={() => setViewMode('calendar')} className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${viewMode === 'calendar' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>日曆</button></div></div></div>
...
const ManualBookingModal: React.FC<{ onClose: () => void, onRefresh: () => void }> = ({ onClose, onRefresh }) => {
    ...
    useEffect(() => {
        ...
        const fetchAvailability = async () => {
            ...
            const { data: special } = await supabase.from('special_dates').select('*');
            const { data: occupied } = await supabase.from('appointments').select('booking_time').eq('booking_date', date).neq('status', 'cancelled');

            if (!rules || !business) return;
            const r = rules.value;
            const day = new Date(date).getDay();
            const spec = special?.find(s => s.date === date);
            
            if (spec?.is_closed) { setAvailableSlots([]); return; }
            
            const hours = business.find(b => b.day_of_week === day);
            if (!hours?.is_open) { setAvailableSlots([]); return; }

            const slots: string[] = [];
            let curr = parseTime(hours.start_time);
            const end = parseTime(hours.end_time);
            const step = r.time_slot_minutes || 60;
            const capacity = r.max_concurrent_bookings || 1;

            while (curr < end) {
                const count = occupied?.filter(o => parseTime(o.booking_time.slice(0,5)) === curr).length || 0;
                if (count < capacity) slots.push(formatTime(curr));
                curr += step;
            }
