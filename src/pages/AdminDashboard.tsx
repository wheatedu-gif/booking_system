...
import { Plus, Trash2, Save, Settings, Users, Calendar as CalendarIcon, FormInput, Clock, LayoutTemplate, List, ChevronLeft, ChevronRight, Lock, AlertCircle, Download, Send, Edit3, X, TrendingUp, Search, ExternalLink, LayoutDashboard } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const updateAppointmentStatus = async (id: string, status: string, reason?: string) => {
    ...
    if (error) showToast('更新失敗', 'error');
      ...
      showToast(status === 'confirmed' ? '已確認預約並發送通知' : '已取消預約');
...
const DashboardHome: React.FC<{ appointments: Appointment[] }> = ({ appointments }) => {
    const stats = {
        ...
        newThisWeek: appointments.filter(a => parseISO(a.created_at) > subMonths(new Date(), 1)).length
    };

    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div>
                <h2 className="text-3xl font-black text-slate-800">早安，管理員 ✌️</h2>
                <p className="text-slate-400 mt-1 font-medium">這是您今天的營運概況。</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard icon={<CalendarIcon />} title="今日預約" value={stats.today} bgColor="bg-blue-50" textColor="text-blue-600" />
                <StatCard icon={<AlertCircle />} title="待處理" value={stats.pending} bgColor="bg-amber-50" textColor="text-amber-600" />
                <StatCard icon={<TrendingUp />} title="已完成預約" value={stats.total} bgColor="bg-green-50" textColor="text-green-600" />
                <StatCard icon={<Users />} title="本月新客" value={stats.newThisWeek} bgColor="bg-purple-50" textColor="text-purple-600" />
            </div>

            <div className="bg-slate-50 rounded-[2.5rem] p-10 border border-slate-100">
                <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2"><Clock className="text-blue-600"/> 最近的預約</h3>
                <div className="space-y-4">
                    {appointments.slice(0, 5).map(apt => (
                        <div key={apt.id} className="bg-white p-5 rounded-2xl flex items-center justify-between shadow-sm border border-white hover:border-blue-100 transition-all">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center font-bold text-slate-400">{(apt as any).customers?.full_name?.[0]}</div>
                                <div>
                                    <div className="font-bold text-slate-800">{(apt as any).customers?.full_name}</div>
                                    <div className="text-xs text-slate-400">{apt.booking_date} {apt.booking_time.slice(0,5)}</div>
                                </div>
                            </div>
                            <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-full ${apt.status === 'confirmed' ? 'bg-green-100 text-green-600' : 'bg-amber-100 text-amber-600'}`}>{apt.status}</span>
                        </div>
                    ))}
                </div>
            </div>
...
const StatCard: React.FC<{ icon: React.ReactNode, title: string, value: number, bgColor: string, textColor: string }> = ({ icon, title, value, bgColor, textColor }) => (
    <div className="bg-white p-8 rounded-[2rem] border border-slate-50 shadow-sm flex flex-col gap-4">
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${bgColor} ${textColor}`}>{icon}</div>
        <div>
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 border-b border-slate-100 pb-6">
        <div className="relative w-full md:w-96">
            <input type="text" placeholder="搜尋姓名、Email 或日期..." className="input-field pl-12 py-3 bg-slate-50 border-none rounded-2xl w-full focus:bg-white transition-all shadow-inner" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        <div className="overflow-hidden border border-slate-100 rounded-3xl">
            ...
            <tbody className="divide-y divide-slate-50">
                ...
                <tr key={apt.id} className="hover:bg-slate-50/50 transition-colors group cursor-pointer" onClick={() => setSelectedApt(apt)}>
                    ...
                    <td className="py-5 px-6"><div className="font-bold text-slate-700 group-hover:text-blue-600">{(apt as any).customers?.full_name}</div><div className="text-slate-400 text-xs">{(apt as any).customers?.email}</div></td>
                    <td className="py-5 px-6"><span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${apt.status === 'confirmed' ? 'bg-green-100 text-green-700' : apt.status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{apt.status === 'confirmed' ? '已確認' : apt.status === 'cancelled' ? '已取消' : '待處理'}</span></td>
                    <td className="py-5 px-6 text-right" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            {apt.status !== 'cancelled' && <button onClick={() => { const r = window.prompt('原因'); if(r !== null) onStatusChange(apt.id, 'cancelled', r); }} className="text-slate-400 hover:text-red-500 font-bold text-xs">取消</button>}
                        </div>
                    </td>
                </tr>))}
            </tbody>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white rounded-[3rem] w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                <div className="bg-slate-900 p-10 text-white flex justify-between items-start">
                    <div><div className="text-blue-400 text-[10px] font-black uppercase tracking-[0.2em] mb-2">預約詳情</div><h3 className="text-3xl font-black">{customer?.full_name}</h3><p className="text-slate-400 mt-1">{customer?.email}</p></div>
                    <button onClick={onClose} className="p-3 bg-white/10 hover:bg-white/20 rounded-2xl transition-all"><X /></button>
                </div>
                <div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
                    <section className="space-y-6">
                        <div className="flex gap-4 items-start"><Clock className="text-blue-600 mt-1"/><div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">預約時間</div><div className="text-lg font-bold">{apt.booking_date} {apt.booking_time.slice(0,5)}</div></div>
                        <div className="flex gap-4 items-start"><AlertCircle className="text-blue-600 mt-1"/><div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">當前狀態</div><div className="text-lg font-bold uppercase">{apt.status}</div></div>
                    </section>
                    <section className="bg-slate-50 rounded-3xl p-6">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">客戶填寫內容</h4>
                        <div className="space-y-3">
                            {Object.entries(apt.booking_data || {}).map(([k, v]) => (
                                <div key={k} className="flex justify-between text-sm border-b border-slate-200/50 pb-2"><span className="text-slate-500">{k}</span><span className="font-bold text-slate-800">{String(v)}</span></div>
                            ))}
                        </div>
                    </section>
                </div>
                <div className="bg-slate-50 p-8 flex justify-end gap-4">
                    {apt.status === 'pending' && <button onClick={() => { onStatusChange(apt.id, 'confirmed'); onClose(); }} className="bg-green-600 text-white px-8 py-3 rounded-2xl font-black shadow-lg shadow-green-200 hover:bg-green-700 transition-all">確認預約</button>}
                    {apt.status !== 'cancelled' && <button onClick={() => { const r = window.prompt('原因'); if(r!==null) { onStatusChange(apt.id, 'cancelled', r); onClose(); } }} className="bg-white text-red-500 border border-red-100 px-8 py-3 rounded-2xl font-black transition-all">取消預約</button>}
                    <button onClick={onClose} className="text-slate-400 font-bold px-4 hover:text-slate-600">關閉</button>
                </div>
            </div>
        </div>
    );
...
const AppointmentCalendar: React.FC<{ appointments: Appointment[], onStatusChange: (id: string, s: string, reason?: string) => void, onSelect: (a: Appointment) => void }> = ({ appointments, onStatusChange, onSelect }) => {
  ...
  return (
    <div className="bg-white animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-7 mb-4 text-center text-[10px] font-black text-slate-400 tracking-[0.2em] uppercase">{['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => <div key={d}>{d}</div>)}</div>
      <div className="grid grid-cols-7 gap-px bg-slate-100 border border-slate-100 rounded-[2.5rem] overflow-hidden shadow-sm">
        {calendarDays.map((day, idx) => (
          <div key={idx} className={`min-h-[140px] p-3 bg-white ${isSameMonth(day, monthStart) ? '' : 'bg-slate-50/50 opacity-40'}`}>
            ...
            <div className="space-y-1.5">{getDayAppointments(day).map(apt => <div key={apt.id} className={`text-[10px] p-2 rounded-xl border-l-4 shadow-sm truncate font-bold cursor-pointer hover:scale-105 transition-transform ${apt.status === 'confirmed' ? 'bg-green-50 text-green-700 border-green-500' : 'bg-blue-50 text-blue-700 border-blue-500'}`} onClick={() => onSelect(apt)}>{apt.booking_time.slice(0,5)} {(apt as any).customers?.full_name}</div>)}</div>
          </div>
        ))}
      </div>
    </div>
...
const SettingsManager: React.FC = () => {
  ...
  const saveSettings = async () => { await supabase.from('system_settings').upsert({ key: 'email_config', value: config }); showToast('系統設定已儲存'); };
  ...
  return (
    <div className="space-y-8 max-w-lg animate-in fade-in duration-500">
      ...
      <div className="bg-slate-50 p-10 rounded-[2.5rem] space-y-8 border border-slate-100 shadow-sm">
        ...
        <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block ml-1">Gmail 帳號</label><input className="input-field bg-white" value={config.user} onChange={e => setConfig({...config, user: e.target.value})} /></div>
  ...
  return (
    <div className="space-y-8 relative animate-in fade-in duration-500">
      ...
      <div className="grid gap-4">{customers.map(c => (
          <div key={c.id} className="p-6 bg-slate-50/50 rounded-[2rem] flex justify-between items-center border border-slate-50 hover:bg-white hover:shadow-xl hover:shadow-slate-100 transition-all group"><div className="flex items-center gap-5"><div className="w-14 h-14 bg-blue-100 text-blue-600 rounded-2xl flex items-center justify-center font-black text-xl shadow-inner shadow-blue-200/50">{c.full_name[0]}</div><div><div className="font-bold text-lg text-slate-800 group-hover:text-blue-600 transition-colors">{c.full_name}</div><div className="text-xs text-slate-400 font-bold">{c.email} {c.phone && `| ${c.phone}`}</div></div></div><button onClick={() => setEditingCustomer(c)} className="opacity-0 group-hover:opacity-100 transition-all p-3 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl"><Edit3 size={20} /></button></div>
        ))}</div>
      {editingCustomer && (
