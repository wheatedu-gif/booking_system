...
import { Plus, Trash2, Save, Settings, Users, Calendar as CalendarIcon, FormInput, Clock, LayoutTemplate, List, ChevronLeft, ChevronRight, Lock, AlertCircle, Download, Send, Edit3, X, TrendingUp, Search, ExternalLink, LayoutDashboard, UserPlus } from 'lucide-react';
...
const StatCard: React.FC<{ icon: React.ReactNode, title: string, value: number, color: string }> = ({ icon, title, value, color }) => {
    // 修正：顯式定義類別名稱，避免 Tailwind 樹搖晃 (Tree Shaking) 導致動態類別失效
    const colorMap: any = {
...
const ManualBookingModal: React.FC<{ onClose: () => void, onRefresh: () => void }> = ({ onClose, onRefresh }) => {
    ...
    useEffect(() => {
        supabase.from('customers').select('id, full_name, email').then(({ data }) => setCustomers(data || []));
    }, []);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        else { showToast('預約已建立'); onRefresh(); onClose(); }
        setLoading(false);
    };
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
            <div className="bg-white rounded-[2.5rem] w-full max-w-md p-10 shadow-2xl animate-in zoom-in-95 duration-200">
                <div className="flex justify-between items-center mb-8"><h3 className="text-2xl font-black text-slate-800">手動建立預約</h3><button onClick={onClose} className="p-2 text-slate-400 hover:bg-slate-100 rounded-full transition-all"><X /></button></div>
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1">選擇現有會員</label><select className="input-field rounded-2xl py-4 bg-slate-50 border-none" value={selectedCustomer} onChange={e => setSelectedCustomer(e.target.value)} required><option value="">請選擇客戶...</option>{customers.map(c => <option key={c.id} value={c.id}>{c.full_name} ({c.email})</option>)}</select></div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1">日期</label><input type="date" className="input-field rounded-2xl py-4 bg-slate-50 border-none" value={date} onChange={e => setDate(e.target.value)} required /></div>
                        <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1">時間</label><input type="time" className="input-field rounded-2xl py-4 bg-slate-50 border-none" value={time} onChange={e => setTime(e.target.value)} required /></div>
                    </div>
...
// ... (其餘組件 FormManager, SettingsManager, CustomerManager 保持不變) ...
...
const FormManager: React.FC<{ formDefs: FormDefinition[], onRefresh: () => void }> = ({ formDefs, onRefresh }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500"><h2 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3"><FormInput className="text-blue-600" /> 表單自定義管理</h2>{!editingDef ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">{formDefs.map(def => (
            ...
            ))}</div><button onClick={() => { const n: FormField = { id: Math.random().toString(36).substr(2,9), name: `f_${Date.now()}`, label: '新欄位', type: 'text', required: false }; setEditingDef({ ...editingDef, fields: [...editingDef.fields, n] }); }} className="w-full py-8 border-2 border-dashed border-slate-200 rounded-[2.5rem] text-slate-400 font-bold hover:bg-blue-50 hover:text-blue-500 hover:border-blue-200 transition-all flex items-center justify-center gap-3">增加更多欄位</button></div>)}</div>
  );
};
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="bg-slate-50 p-10 rounded-[2.5rem] border border-slate-100 shadow-sm max-w-2xl">
        {activeSubTab === 'smtp' ? (
            <div className="space-y-6"><label className="flex items-center gap-5 p-5 bg-white rounded-3xl cursor-pointer border border-slate-100 hover:border-blue-200 transition-all shadow-sm"><input type="checkbox" className="w-6 h-6 text-blue-600 rounded-xl" checked={config.enabled} onChange={e => setConfig({...config, enabled: e.target.checked})} /><div className="flex-1"><div className="font-bold text-slate-700">啟用 Email 自動通知</div></div></label><div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block ml-1">寄件者名稱</label><input className="input-field bg-white" value={config.from_name} onChange={e => setConfig({...config, from_name: e.target.value})} /></div><div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block ml-1">Gmail 帳號</label><input className="input-field bg-white" value={config.user} onChange={e => setConfig({...config, user: e.target.value})} /></div><div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block ml-1">Gmail 應用程式密碼</label><input type="password" placeholder="16 位密碼" className="input-field bg-white" value={config.pass} onChange={e => setConfig({...config, pass: e.target.value})} /></div></div>
        ) : (
