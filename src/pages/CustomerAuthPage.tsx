...
import { Calendar, User, Mail, Lock, AlertCircle } from 'lucide-react';
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center p-4 bg-slate-50">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 border border-slate-100">
        <div className="flex flex-col items-center mb-8">
          <div className="bg-green-600 p-3 rounded-xl text-white mb-4 shadow-lg shadow-green-200">
            <User size={32} />
          </div>
          <h2 className="text-2xl font-bold text-slate-800">
            {isSignUp ? '註冊會員帳號' : '會員登入'}
          </h2>
          <p className="text-slate-500 text-sm mt-2">
            {isSignUp ? '註冊後即可開始預約服務' : '請輸入您的帳號密碼'}
          </p>
        </div>
        ...
        <div className="mt-4 text-center">
            <a href="/admin/login" className="text-slate-400 text-xs hover:text-slate-600">我是管理員？點此登入後台</a>
        </div>
      </div>
    </div>
  );
