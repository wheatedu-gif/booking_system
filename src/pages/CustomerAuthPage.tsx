            ...
            <>
              ...
              {formDef?.fields.filter(f => !f.isSystem).map((field) => (
                <div key={field.id}>
                  ...
                  <div className="relative">{field.type === 'select' ? (<select required={field.required} className="input-field pl-12 py-4 rounded-2xl bg-slate-50 border-none appearance-none" value={dynamicData[field.label] || ''} onChange={(e) => setDynamicData({ ...dynamicData, [field.label]: e.target.value })}><option value="">請選擇...</option>{field.options?.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select>) : field.type === 'textarea' ? (<textarea required={field.required} className="input-field pl-12 py-4 rounded-2xl bg-slate-50 border-none shadow-inner min-h-[100px]" value={dynamicData[field.label] || ''} onChange={(e) => setDynamicData({ ...dynamicData, [field.label]: e.target.value })} placeholder={field.label} />) : (<input type={field.type || 'text'} required={field.required} className="input-field pl-12 py-4 rounded-2xl bg-slate-50 border-none shadow-inner" value={dynamicData[field.label] || ''} onChange={(e) => setDynamicData({ ...dynamicData, [field.label]: e.target.value })} placeholder={field.label} />)}<FileText size={20} className="absolute left-4 top-4 text-slate-300" /></div>
                </div>
              ))}
